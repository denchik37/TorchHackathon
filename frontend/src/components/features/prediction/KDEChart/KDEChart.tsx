...
import { KDEChartModal } from '../KDEChartModal';
...
export const KDEChart = forwardRef<KDEChartRef, KDEChartProps>(
  (
    ...
    const [zoomTransform, setZoomTransform] = useState<d3.ZoomTransform | undefined>(
    ...
    useEffect(() => {
      ...
      const margin = { top: 15, right: 0, bottom: 35, left: 65 };
      ...
      gradient.append('stop').attr('offset', '50%').attr('stop-color', 'rgba(59, 130, 246, 0.4)');
      ...
      const x = d3
        .scaleTime()
        .domain([
          new Date(minTime.getTime() - timePadding),
          new Date(maxTime.getTime() + timePadding),
        ])
        .range([0, width]);
      ...
      if (enableZoom) {
        const zoom = d3
          ...
          .on('zoom', (event) => {
            ...
            (svg.select('.x-axis') as any).call(d3.axisBottom(x).scale(transform.rescaleX(x)));
            (svg.select('.y-axis') as any).call(d3.axisLeft(y).scale(transform.rescaleY(y)));

            (svg.select('.grid-x') as any).call(
              d3
                .axisBottom(x)
                .scale(transform.rescaleX(x))
                .tickSize(-height)
                .tickFormat(() => '')
            );
      ...
      const lowDensityColor = d3
        .scaleSequential(d3.interpolateRgb('rgba(59, 130, 246, 0.1)', 'rgba(59, 130, 246, 0.3)'))
        .domain([0, maxDensityValue * 0.3]);

      const mediumDensityColor = d3
        .scaleSequential(d3.interpolateRgb('rgba(59, 130, 246, 0.3)', 'rgba(59, 130, 246, 0.6)'))
        .domain([maxDensityValue * 0.3, maxDensityValue * 0.7]);

      const highDensityColor = d3
        .scaleSequential(d3.interpolateRgb('rgba(59, 130, 246, 0.6)', 'rgba(59, 130, 246, 0.9)'))
    ...
    return (
      <div className={cn(kdeChartStyles.root, className)}>
        <div className={kdeChartStyles.header}>
          <div className="flex items-center gap-3">
            <span className={kdeChartStyles.headerTitle}>HBAR price × time probability</span>
            <span className={kdeChartStyles.headerPrice}>${currentPrice.toFixed(4)}</span>
          </div>
          <div className={kdeChartStyles.controlsWrap}>
            {enableZoom && showControls && (
              <>
                <Button variant="outline" size="sm" onClick={handleZoomOut} className={kdeChartStyles.controlsButton} title="Zoom out">
                  <ZoomOut className="h-3.5 w-3.5" />
                </Button>
                <Button variant="outline" size="sm" onClick={handleZoomIn} className={kdeChartStyles.controlsButton} title="Zoom in">
                  <ZoomIn className="h-3.5 w-3.5" />
                </Button>
                <Button variant="outline" size="sm" onClick={handleResetZoom} className={kdeChartStyles.controlsButton} title="Reset zoom">
                  <RotateCcw className="h-3.5 w-3.5" />
                </Button>
                <Button variant={isPanning ? 'default' : 'outline'} size="sm" onClick={togglePan} className={kdeChartStyles.controlsButton} title="Pan">
                  <Move className="h-3.5 w-3.5" />
                </Button>
              </>
            )}
            {showControls && (
              <Button variant="outline" size="sm" onClick={() => setIsModalOpen(true)} className={kdeChartStyles.expandButton}>
