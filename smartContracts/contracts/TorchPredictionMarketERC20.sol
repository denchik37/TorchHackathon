...
/**
 * @title TorchPredictionMarketERC20
 * @dev A prediction market for TORCH token price with quality-based weighting; collateral is an ERC20
 * @notice Users approve this contract then place bets with the configured token
 */
contract TorchPredictionMarketERC20 is Ownable {
    using SafeERC20 for IERC20;

    /// @notice ERC20 used for stakes, fees, and payouts
    ...
    constructor(IERC20 _collateralToken) {
        require(address(_collateralToken) != address(0), "Invalid token");
    ...
    /**
     * @notice Place a bet with collateral ERC20 (pulls `stakeAmount` via transferFrom)
     ...
     * @param stakeAmount Gross stake before fee (fee taken from this amount)
    ...
    function placeBatchBets(
        ...
        // Calculate total stake needed and validate each bet
        for (uint256 i = 0; i < targetTimestamps.length; i++) {
            ...
            require(stakeAmounts[i] > 0, "Zero stake");

            totalStake += stakeAmounts[i];
        }

        collateralToken.safeTransferFrom(msg.sender, address(this), totalStake);
    ...
    /**
     * @notice Emergency withdraw all collateral token (only owner)
     */
    ...
    function simulatePlaceBet(
        ...
        if (stakeAmount == 0) {
            return BetSimulation({
                fee: 0, stakeNet: 0, sharpnessBps: 0, timeBps: 0, qualityBps: 0, weight: 0, bucket: 0,
                isValid: false, errorMessage: "Zero stake"
            });
        }
    ...
    function getStats() external view returns (
        ...
        return (nextBetId, totalFeesCollected, collateralToken.balanceOf(address(this)));
    }

    /// @dev Reject native token; only ERC20 collateral is supported
